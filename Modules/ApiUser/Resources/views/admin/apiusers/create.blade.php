...
@section('content-header')
    <h1>
        {{ trans('apiuser::apiusers.title.create apiuser') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.apiuser.apiuser.index') }}">{{ trans('apiuser::apiusers.title.apiusers') }}</a></li>
        <li class="active">{{ trans('apiuser::apiusers.title.create apiuser') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::open(['route' => ['admin.apiuser.apiuser.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('apiuser::admin.apiusers.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
